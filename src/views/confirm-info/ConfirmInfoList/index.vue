    ...
    methods: {
        ...
        selectChange(val) {
            if (val === "timeField" && this.searchQuery.year) {
                this.searchQuery.year = this.$options.filters["date"](
                    this.searchQuery.year.getTime(),
                    "yyyy"
                );
            }
        ...
        getConfirmMemberList() {
            api[urlNames["getConfirmMemberList"]]().then(
                (res) => {
        ...
        getGrid() {
            ...
            api[urlNames["getInfoConfirmList"]](data).then(
                (res) => {
        ...
        goFindDetial() {
            this.$router.push(
                `/organization/organization-content/${this.app.option.user.orgNodeId}`
            );
        },
        handleConfirm() {
            let newDatefmt = this.$options.filters["date"](
                new Date().getTime(),
                "yyyy-MM"
            );
            let date = newDatefmt.split("-").join("");
            if (date) {
                api[urlNames["insertConfirmInfo"]]({ date: date }).then(
                    (res) => {
                        if (res && res.status === 0) {
