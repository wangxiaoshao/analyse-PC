...
export default {
  ...
  methods: {
    ...
    sort (item) {
      if (item.order) {
        this.page.current = 1
        this.searchForm.sidx = `${item.prop} ${
    ...
    // 过滤
    filter (oFilter) {
      const aFilter = Object.keys(oFilter)
      const length = aFilter.length
    ...
    dateChange (val) {
      this.searchForm.startDate = val
        ? this.$options.filters.date(val[0].getTime(), 'yyyy-MM-dd')
        : ''
      this.searchForm.endDate = val
        ? this.$options.filters.date(val[1].getTime(), 'yyyy-MM-dd')
        : ''
    ...
    exportList (urlKey, fileName) {
      ...
      const data = {}
      const aKey = Object.keys(this.searchForm)
    ...
    bindTableHeight (height = 0) {
      this.tableHeight =
                document.documentElement.clientHeight - 160 - height
