        ...
        <el-table-column prop="name" label="所有下级" width="180px">
          <template slot-scope="scope">
            <span>
              ...
              <span class="iconfont icondanwei orgColor" v-if="scope.row.nodeType === 2"></span>
              <span class="iconfont iconbumen deptColor" v-if="scope.row.nodeType === 3"></span>
              <span class="el-icon-user personColor" v-if="!scope.row.nodeType"></span>
            </span>
            <span
              v-if="!scope.row.nodeType"
            >{{visableData.userName==0 ? hideName(scope.row.name):scope.row.name}}</span>
